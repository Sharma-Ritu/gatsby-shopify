          ...
          <Col sm="4">
            ...
            <li className="d-flex mb-3" key={id}>
                  <div className="recent-post-image col-4">
                      <Link to={`/blogs/${url.split("/").pop()}/`}><img src={image.src} className="img-fluid" alt="Blog w-100" style={{transition:'all 0.15s ease-in-out'}}/></Link>
                  </div>
                  <div className="recente-posts-data col-8 px-0">
                    <p className="mb-0"><Link to={`/blogs/${url.split("/").pop()}/`} style={{fontSize:'12px', color:'#000'}} className="filson-pro-reg d-block mb-0">{title}</Link></p>
